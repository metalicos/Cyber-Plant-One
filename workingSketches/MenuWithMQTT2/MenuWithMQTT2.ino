...
void encLoop() {
  ...
  if (encCounter > 0 && buttonIsPressed) {                                  // енкодер повернутий вправо при зажатій кнопці
    encoder.setCount(0);
    LCDML.BT_right();
  } else if (encCounter < 0 && buttonIsPressed) {                           // енкодер повернутий вліво при зажатій кнопці
    encoder.setCount(0);
    LCDML.BT_left();
