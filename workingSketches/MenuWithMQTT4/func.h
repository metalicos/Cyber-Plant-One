...
void saveAllDataToMemory() {
  ...
  memory.getUShort("channelVal0", cpd.channelValue[0]);
  memory.getUShort("channelVal1", cpd.channelValue[1]);
  memory.getUShort("channelVal2", cpd.channelValue[2]);
  memory.getUShort("channelVal3", cpd.channelValue[3]);
...
void singleClick(Button2& btn) {
  if (!encoderMovedWithPressedButton) {   // якщо прокручували енкодер із НЕ затисненою кнопкою
    LCDML.BT_enter();
    Serial.println("Швидкий клік");
  }
  encoderMovedWithPressedButton = false;  // обов'язково скидуємо прапорець
}
...
void encLoop() {
  ...
  if (encCounter > 0 && buttonIsPressed) {                                  // енкодер повернутий вправо при затиснутій кнопці
    Serial.println("Натиснута кнопка  -  енкодер вправо -> -> -> ->");
    encoderMovedWithPressedButton = true;                                   // забороняє виконуватися функціям обробникам, якщо енкодер прокручували за час більший 1с
    encoder.setCount(0);
    LCDML.BT_right();
  } else if (encCounter < 0 && buttonIsPressed) {                           // енкодер повернутий вліво при затиснутій кнопці
    Serial.println("Натиснута кнопка  -  енкодер вліво <- <- <- <-");
    encoderMovedWithPressedButton = true;                                   // забороняє виконуватися функціям обробникам, якщо енкодер прокручували за час більший 1с
    encoder.setCount(0);
    LCDML.BT_left();
  } else if (encCounter > 0) {                                              // енкодер повернутий вправо
    Serial.println("Енкодер вправо -> -> -> ->");
    encoder.setCount(0);
    LCDML.BT_down();
  } else if (encCounter < 0) {                                              // енкодер повернутий вліво
    Serial.println("Енкодер вліво <- <- <- <-");
    encoder.setCount(0);
    LCDML.BT_up();
  }
}
...
void autoSave() {
  ...
  if (activeHoursCounter == 5) {                                          // що 5 годин записуємо необхідні дані
...
void setupChannelRegulators() {
  for (byte i = 0; i < 4; i++) {
    ...
    if (cpd.channelType[i] == TYPE_DIMMER) {
      ...
      reg[i].setpoint = cpd.maintainValue[i];    // установка (ставим на 40 градусов)
      reg[i].hysteresis = cpd.relayHysteresis[i];   // ширина гистерезиса
      reg[i].k = cpd.relayK[i];          // коэффициент обратной связи (подбирается по факту)
      reg[i].dT = cpd.relayDt[i];       // установить время итерации для getResultTimer
